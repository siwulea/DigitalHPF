...
#define ENC1_RESOLUTION     1800.0 // eQEP2에 사용한 엔코더 Resolution
#define M1_THRESHOLD        20L   // eQEP2을 통한 속도 계산시 M Method와 T Method 사이의 기준이 되는 임계값 M Value
#define T1_THRESHOLD        500L  // eQEP2을 통한 속도 계산시 RPM을 0으로 처리하고자 하는 최대 경과 시간의 임계값
// #define PI 3.141592
#define MUL_MT1   1
#define N_Pole1  4

//#define ENC2_RESOLUTION       4096.0 // eQEP2에 사용한 엔코더 Resolution
#define ENC2_RESOLUTION     1800.0 // eQEP2에 사용한 엔코더 Resolution
#define M2_THRESHOLD        20L   // eQEP2을 통한 속도 계산시 M Method와 T Method 사이의 기준이 되는 임계값 M Value
#define T2_THRESHOLD        500L  // eQEP2을 통한 속도 계산시 RPM을 0으로 처리하고자 하는 최대 경과 시간의 임계값
// #define PI 3.141592
#define MUL_MT2   1
#define N_Pole2  4
...
{
    if(Cal_Offset_Chk == 0)CalibrateADC();              //*ADC 보정값을 구하기 위한 함수

    if(Cal_Offset_Chk == 1)CalculateADCOffset();        //*ADC 입력 시 DC 오프셋 값을 구하는 함수. 여기서 구한 DC 오프셋을 빼주면  AC값만 구할 수 있다.
...
{
    // Insert ISR Code here
    // eQEP2 Unit time out interrupt : 1msec interval
